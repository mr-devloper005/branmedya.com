...
import { ArrowRight, Bookmark, BriefcaseBusiness, Building2, Camera, CheckCircle2, Download, FileText, Filter, Image as ImageIcon, MapPin, Megaphone, Search, UserRound } from 'lucide-react'
...
const taskDeck: Record<TaskKey, { icon: typeof FileText; archiveClass: string; promise: string; badge: string }> = {
  ...
  listing: { icon: Building2, archiveClass: 'grid gap-5', promise: 'Directory results highlight company identity, claim status, location, contact context, and a clear profile action.', badge: 'Business' },
...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  const archiveVars = { '--archive-bg': '#f5f7f5', '--archive-text': '#25211f', '--archive-surface': '#ffffff', '--archive-accent': '#00a651' } as CSSProperties
  const categoryLabel = category === 'all' ? 'All categories' : CATEGORY_OPTIONS.find((item) => item.slug === category)?.name || category

  return (
    <EditableSiteShell>
      <main style={archiveVars} className="bg-[var(--archive-bg)] text-[var(--archive-text)]">
        <section className="mx-auto grid max-w-[var(--editable-container)] gap-6 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_320px] lg:px-0 lg:py-12">
          <div className="rounded-md border border-[var(--editable-border)] bg-[var(--archive-surface)] p-6 shadow-[0_18px_50px_rgba(15,23,42,0.06)] sm:p-8">
            <div className="inline-flex items-center gap-2 rounded-md bg-[#25211f] px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-white"><Icon className="h-4 w-4" /> {label}</div>
            <h1 className="mt-5 max-w-4xl text-4xl font-semibold leading-tight tracking-tight sm:text-5xl">{voice?.headline || `Browse ${label}`}</h1>
            <p className="mt-5 max-w-2xl text-base leading-8 text-slate-600">{voice?.description || SITE_CONFIG.description}</p>
            <div className="mt-5 rounded-md border border-[var(--editable-border)] bg-[#f5f7f5] p-4 text-sm font-semibold leading-7 text-slate-700">{deck.promise}</div>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={basePath} className="rounded-md bg-[#00a651] px-5 py-3 text-sm font-black text-white">Browse all</Link>
          ...
          <form action={basePath} className="self-end rounded-md border border-[var(--editable-border)] bg-white p-5 shadow-sm">
            ...
            <button className="mt-3 h-12 w-full rounded-md bg-[#00a651] text-sm font-black text-white">Apply</button>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 pb-16 sm:px-6 lg:px-0">
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            ...
            <span className="rounded-md bg-[#00a651] px-5 py-3 text-sm font-black text-white">Page {page} of {pagination.totalPages || 1}</span>
...
function ListingArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group grid gap-5 rounded-md border border-black/10 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-[#00a651]/45 hover:shadow-[0_18px_42px_rgba(15,23,42,0.10)] sm:grid-cols-[160px_minmax(0,1fr)_auto]">
      <div className="flex h-36 w-full items-center justify-center overflow-hidden rounded-md bg-black text-4xl font-semibold text-white sm:w-40">
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-md bg-[#f0f2f0] px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-slate-600">{category}</span>
        </div>
        <h2 className="mt-3 text-3xl font-semibold leading-tight tracking-tight">{post.title}</h2>
        {location ? <p className="mt-2 flex items-start gap-2 text-base font-medium text-slate-800"><MapPin className="mt-1 h-4 w-4 shrink-0 text-[#00a651]" /> {location}</p> : null}
        <p className="mt-5 line-clamp-2 max-w-2xl text-sm leading-6 text-slate-600">{getSummary(post) || `${post.title} is listed in the business directory with service, location, and profile details ready for comparison.`}</p>
        <div className="mt-5 flex flex-wrap gap-2 text-xs font-bold text-slate-600">
          {phone ? <span className="rounded-md bg-[#f5f7f5] px-3 py-2">Phone: {phone}</span> : null}
          {website ? <span className="rounded-md bg-[#f5f7f5] px-3 py-2">Website available</span> : null}
        </div>
      </div>
